...
import { useExamUpdatesForJob } from "@/hooks/useExamUpdates";
...
export default function JobDetails() {
  ...
  const { data: examUpdates } = useExamUpdatesForJob(job?.id, job?.title);
  ...
  const handleTrackExam = async () => {
    ...
    try {
      ...
      // If exam doesn't exist, create it with job_id link
      if (!examId) {
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <main className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        ...
        {examUpdates && examUpdates.length > 0 && (
          <ExamUpdatesSection updates={examUpdates} />
        )}
...
/** Scraped Exam Updates section — shows important dates, download links, and sections from exam_updates */
function ExamUpdatesSection({ updates }: { updates: ExamUpdateItem[] }) {
  // Aggregate important dates & download links from all matching updates
  const allDates: { event: string; date: string; status: string; link: string; source: string }[] = [];
  const allDownloads: { text: string; url: string; category: string }[] = [];
  ...
  const categoryColor = (cat: string) => {
    const colors: Record<string, string> = {
      admit_card: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300",
      result: "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300",
      answer_key: "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300",
      syllabus: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300",
    };
    return colors[cat] || "bg-secondary text-muted-foreground";
  };

  return (
    <div className="space-y-4 animate-slide-up" style={{ animationDelay: "0.85s" }}>
      {/* Section Header */}
        ...
        <Card className="border border-border/40 bg-card/60 backdrop-blur-md shadow-card rounded-2xl">
          <CardContent className="p-4 space-y-3">
            ...
            {summaries.slice(0, 5).map((s, i) => (
              <div key={i} className="flex items-start gap-3 bg-secondary/30 rounded-lg px-3 py-2.5">
                <Badge className={`text-[10px] flex-shrink-0 ${categoryColor(s.category)} border-0`}>
                  {categoryLabel(s.category)}
                </Badge>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-foreground line-clamp-1">{s.title}</p>
                  <p className="text-xs text-muted-foreground line-clamp-2 mt-0.5">{s.summary}</p>
                </div>
              </div>
            ))}
          </CardContent>
      ...
      {allDownloads.length > 0 && (
        <Card className="border border-border/40 bg-card/60 backdrop-blur-md shadow-card rounded-2xl">
          <CardContent className="p-4">
            ...
            <div className="space-y-2">
              {allDownloads.slice(0, 10).map((dl, i) => (
                <a
                  key={i}
                  href={dl.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-between bg-secondary/30 hover:bg-secondary/50 rounded-lg px-4 py-2.5 transition-colors group"
                >
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <Badge className={`text-[10px] border-0 flex-shrink-0 ${categoryColor(dl.category)}`}>
                      {categoryLabel(dl.category)}
                    </Badge>
                    <span className="text-sm text-foreground truncate">{dl.text}</span>
                  </div>
