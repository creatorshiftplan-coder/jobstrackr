...
export type ExamStatusType = 'result' | 'admit_card' | 'exam_scheduled' | 'admit_pending' | 'date_change' | 'notification' | 'upcoming';
...
export function getExamStatusType(aiData: any): ExamStatusType {
    ...
    const admitReleasedKeywords = [
        ...
        "tier 1 admit", "tier-1 admit", "tier 2 admit", "tier-2 admit",
    ];

    const hasAdmitInStatus = statusText.includes("admit") || summaryText.includes("admit") ||
        statusText.includes("hall ticket") || summaryText.includes("hall ticket");

    const isAdmitReleased = hasAdmitInStatus && (
        combinedText.includes("released") || combinedText.includes("out") ||
        combinedText.includes("available") || combinedText.includes("download")
    );

    if (admitReleasedKeywords.some(kw => combinedText.includes(kw)) || isAdmitReleased) {
        return 'admit_card';
    }
